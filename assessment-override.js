(function(){
  ...
  function normalize(x){return String(x||'').toLowerCase().replace(/[^a-z0-9]+/g,' ').trim();}
  function fieldValue(saved,label){const wanted=normalize(label);for(const [k,v] of Object.entries(saved||{})){const nk=normalize(k);if(nk===wanted||nk.endsWith(' '+wanted)||nk.includes(wanted))return String(v||'').trim();}return '';}
  ...
  function intelligence(){
    ...
    const rom=p.rom||'—';
    const documented=fieldValue(s,'Key impairments');
    const impression=fieldValue(s,'Working clinical impression');
    const functional=fieldValue(s,'Activity limitations')||fieldValue(s,'Participation restrictions');
    ...
    if(c.includes('low back')){
      ...
      why=[documented?`Documented impairment: ${documented}.`:'Document lumbar movement and trunk/core impairments to guide loading.',functional?`Functional impact documented: ${functional}.`:'Link impairments to sitting, bending and other meaningful activities.'];
      ...
      why=[documented?`Documented impairment: ${documented}.`:'Document knee ROM, quadriceps capacity and lower-limb impairments to guide loading.',functional?`Functional impact documented: ${functional}.`:'Link impairments to stairs, transfers and other meaningful activities.'];
      ...
      why=[documented?`Documented impairment: ${documented}.`:'Document ROM, strength and movement-related impairments to guide loading.',functional?`Functional impact documented: ${functional}.`:'Link impairments to overhead and other meaningful activities.'];
      ...
      why=[documented?`Documented impairment: ${documented}.`:'Recommendations remain broad until sufficient patient-specific examination data are documented.',functional?`Functional impact documented: ${functional}.`:'Document meaningful activity and participation limitations.'];
    ...
    return `<div class="ai intelligence-v2">
      ...
      <div class="grid2" style="margin-top:16px"><div class="card" style="box-shadow:none"><h4>Clinical priorities</h4><ul>${priorities.map(x=>`<li>${x}</li>`).join('')}</ul></div><div class="card" style="box-shadow:none"><h4>Suggested treatment direction</h4><ul>${suggestions.map(x=>`<li>${x}</li>`).join('')}</ul></div></div>
      <div class="card intel-reason" style="box-shadow:none"><h4>Why MOVIA suggests this</h4><ul>${why.map(x=>`<li>${esc(x)}</li>`).join('')}</ul></div>
      ...
      <div class="grid2" style="margin-top:16px"><div class="card" style="box-shadow:none"><h4>Safety & screening</h4><p>${safety}</p></div><div class="card" style="box-shadow:none"><h4>Monitor at follow-up</h4><ul>${monitor.map(x=>`<li>${x}</li>`).join('')}</ul></div></div>
  ...
  assessment=function(){
    const p=patient(),saved=load();let comprehensive='';
  ...
  document.addEventListener('click',function(e){
    ...
    if(decision){const label=decision==='approve'?'Approved for treatment plan':'Needs modification';localStorage.setItem(decisionKey(),JSON.stringify({label,time:new Date().toISOString()}));const status=e.target.closest('.intel-approval')?.querySelector('.decision-status');if(status)status.innerHTML=`Last decision: <b>${esc(label)}</b>`;toast(`MOVIA decision recorded: ${label}`);}
  });
})();